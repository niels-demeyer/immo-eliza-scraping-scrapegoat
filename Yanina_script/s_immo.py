...
# FLOOR
elements_floor = soup.select(
    '#main-content > div:nth-child(4) > section > div > div > div > div:nth-child(3) > div:nth-child(2) > span')
# works for both houses and appartments


if not elements_floor:  # Check if the list is empty
    print("-")
else:
    for element in elements_floor:
        # Extracting the text and stripping any leading/trailing whitespace from each matched element
        data_floor = element.text.strip()
        # Using regular expression to find all numbers in the string
        numbers = re.findall(r'\d+[\.,]?\d*', data_floor)

        # Joining the numbers back together, if needed, or just printing the first found number
        numeric_data = '-'.join(numbers) if numbers else '-'
